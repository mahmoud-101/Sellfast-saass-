...
export class CampaignOrchestrator {
    ...
    /**
     * Resiliently parse JSON — delegates to centralized parseRobustJSON
     */
    static safeJsonParse(input: string, fallback: any = null): any {
        return parseRobustJSON(input, fallback);
    }
...
JSON: {"concept":"الفكرة البصرية","backgrounds":["خلفية 1","خلفية 2","خلفية 3"],"props":["إكسسوار 1","إكسسوار 2"],"shots":[{"name":"اسم","setup":"إعداد","mood":"مزاج"}],"colors":"لوحة الألوان","lighting":"نوع الإضاءة"} JSON فقط.`,
    ...
    /**
     * Run the Launch Strategy phase (Gemini-First, with Perplexity/Grok fallback).
    ...
    static async runLaunchBrief(context: ProductIntelligenceData): Promise<OrchestrationResult> {
        try {
            // 1. Gemini: Primary Strategic Depth & Targeting
            const strategyResult = await askGemini(
                `Create a high-end marketing strategy and targeting plan for ${context.productName} in ${context.targetMarket}. 
                Dialect: ${context.dialect}. Product Description: ${context.productDescription}.
                Return JSON structure: {
                  "meta": {"interests": []}, 
                  "tiktok": {"interests": []},
                  "analysis": "Executive strategy summary",
                  "targetAudience": {"demographics": "", "psychographics": ""}
                }`,
                "You are a performance marketing director and strategic expert."
            );

            const strategy = parseRobustJSON(strategyResult, {});

            // 2. Secondary/Real-time enrichment (Perplexity + Grok fallbacks if Gemini is thin)
            const [realTimeTargeting, strategyGrok] = await Promise.allSettled([
                // Perplexity: optional live keywords if the key exists
                askPerplexityJSON(
                    `Live check: Specific high-converting interest keywords for ${context.productName} in ${context.targetMarket} right now.`,
                    "Marketing researcher."
                ),
                // Grok: Strategic depth fallback
                runGrokStrategy(context.productDescription, context.targetMarket, context.dialect)
            ]);

            const targeting = {
                ...strategy.meta ? { meta: strategy.meta } : {},
                ...strategy.tiktok ? { tiktok: strategy.tiktok } : {},
                ...realTimeTargeting.status === 'fulfilled' ? realTimeTargeting.value : {}
            };

            const strategicDepth = {
                ...strategy.analysis ? { analysis: strategy.analysis } : {},
                ...strategy.targetAudience ? { targetAudience: strategy.targetAudience } : {},
                ...strategyGrok.status === 'fulfilled' ? strategyGrok.value : {}
            };

            // 3. Finalize captions (Gemini)
            const captions = await askGemini(
                `Create 3 high-converting captions based on the strategy. 
                Product: ${context.productName}. 
                Strategy Summary: ${strategicDepth.analysis}.
                Insights: ${JSON.stringify(targeting)}.
                JSON array: [{"platform": "Instagram", "text": "...", "hashtags": []}]`,
                "You are an expert Arabic copywriter."
            );
