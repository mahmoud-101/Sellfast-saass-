...
const ProModeDashboard: React.FC<ProModeDashboardProps> = ({ userId, onUpscale }) => {
    ...
    const startPipeline = async () => {
        ...
        try {
            ...
            console.log("[ProMode] Starting Market Analysis...");
            const marketData = await agentMarketAnalyzer(productForm);
            console.log("[ProMode] Market Data received:", marketData);
            setPipeline(prev => ({ ...prev, status: 'strategizing', marketAnalysis: marketData }));

            // 2. Agent Angle Strategist
            console.log("[ProMode] Starting Angle Strategist...");
            const anglesData: AgentAngle[] = await agentAngleStrategist(productForm, marketData);
            console.log("[ProMode] Angles Data received:", anglesData);
            setPipeline(prev => ({ ...prev, status: 'hooking', angles: anglesData }));

            // 3. Parallel Processing for Hooks, Copy, and Visuals (Per Angle)
            let safeAngles: AgentAngle[] = [];
            if (Array.isArray(anglesData) && anglesData.length > 0) {
                safeAngles = anglesData;
            } else if (anglesData && typeof anglesData === 'object') {
                const innerArray = Object.values(anglesData).find(v => Array.isArray(v));
                if (innerArray) {
                    safeAngles = innerArray as AgentAngle[];
                }
            }

            if (!safeAngles || safeAngles.length === 0) {
                console.error("[ProMode] Fatal: No angles extracted even after fallbacks.");
                throw new Error("عذراً، لم نتمكن من استخراج الزوايا التسويقية حالياً بسبب ضغط عالي على المحركات. من فضلك حاول مرة أخرى بعد دقائق.");
            }

            const topAngles = safeAngles.slice(0, 5);
            ...
            const preliminaryAdsList: FinalProModeAd[] = await Promise.all(
                topAngles.map(async (angle) => {
                    const [rawHooks, rawVisual] = await Promise.all([
                        agentHookWriter(productForm, angle),
                        agentVisualDirector(productForm, angle)
                    ]);

                    let safeHooks = Array.isArray(rawHooks) ? rawHooks : Object.values(rawHooks || {}).find(v => Array.isArray(v)) as string[] || [];
                    if (!safeHooks || safeHooks.length === 0) safeHooks = [angle.title || 'اكتشف السر الآن', 'فرصة لا تعوض', 'جرب بنفسك'];

                    const bestHook = safeHooks[0] || '';
                    const copyData = await agentCopywriter(productForm, angle, bestHook);

                    return {
                        angle,
                        hooks: safeHooks,
                        copy: copyData || { adBody: 'تفاصيل المنتج...', callToAction: 'اطلب الآن' },
                        visual: rawVisual || { imagePrompt: 'Premium product photography', selectedStyleName: 'Pro Style' }
                    };
                })
            );
