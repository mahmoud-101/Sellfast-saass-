...
// 🌐 Original Supabase Instance (for fallback/internal use)
export const supabaseInstance = createClient(
    supabaseUrl || 'https://placeholder-project.supabase.co',
    supabaseAnonKey || 'placeholder-key'
);

// 🧠 Unified Database Access - Prefers InsForge if configured
export const db: any = isInsForgeConfigured() ? insforge : supabaseInstance;

// 🔗 Re-export the unified client as 'supabase' to satisfy all existing imports
...
export const getUserProfile = async (userId: string, email?: string, referredBy?: string) => {
    ...
    try {
        ...
        if (!data) {
            ...
            if (referredBy) {
                const { data: refData } = await db.from('profiles').select('id').eq('referral_code', referredBy).maybeSingle();
...
export const awardPoints = async (userId: string, points: number, reason: string) => {
    ...
    try {
        const { data: profile } = await db.from('profiles').select('user_points').eq('id', userId).single();
...
export const getReferralStats = async (userId: string) => {
    ...
    try {
        ...
        const { count } = await db.from('profiles').select('*', { count: 'exact', head: true }).eq('referred_by', userId);

        // جلب إجمالي النقاط المكتسبة من الإحالات (من السجلات)
        const { data: logs } = await db.from('logs').select('description').eq('user_id', userId).eq('action_type', 'REFERRAL_BONUS');
...
export const getAdminStats = async () => {
    ...
    const { data: creditsData } = await db.from('profiles').select('credits');
...
export const logAction = async (userId: string, type: string, description: string) => {
    ...
    await db.from('logs').insert([{ user_id: userId, action_type: type, description }]);
};
...
export const getGeneratedAssets = async (userId: string) => {
    if (!isSupabaseConfigured()) return [];
    const { data } = await db.from('assets').select('*').eq('user_id', userId).order('created_at', { ascending: false });
    return data || [];
};

export const deleteGeneratedAsset = async (id: string, userId: string) => {
    if (!isSupabaseConfigured()) return;
    await db.from('assets').delete().eq('id', id).eq('user_id', userId);
};
...
export const saveBrandKit = async (userId: string, brandData: any) => {
    if (!isSupabaseConfigured()) return;
    const { data, error } = await db.from('brand_kits').upsert([{
        ...brandData,
        user_id: userId,
...
export const getUserBrandKits = async (userId: string) => {
    if (!isSupabaseConfigured()) return [];
    const { data } = await db.from('brand_kits').select('*').eq('user_id', userId).order('updated_at', { ascending: false });
    return data || [];
};

export const deleteBrandKit = async (id: string) => {
    if (!isSupabaseConfigured()) return;
    await db.from('brand_kits').delete().eq('id', id);
};
...
export const getPendingPayments = async () => {
    if (!isSupabaseConfigured()) return [];
    const { data } = await db.from('payment_requests').select('*, profiles(id, credits)').eq('status', 'pending').order('created_at', { ascending: false });
    return data || [];
};
...
export const approvePayment = async (requestId: string, adminId: string) => {
    ...
    // 1. جلب بيانات الطلب
    const { data: request } = await db.from('payment_requests').select('*').eq('id', requestId).single();
    if (!request) return;

    // 2. تحديث رصيد المستخدم
    const { data: profile } = await db.from('profiles').select('credits').eq('id', request.user_id).single();
    const newCredits = (profile?.credits || 0) + parseInt(request.credits);

    await db.from('profiles').update({ credits: newCredits }).eq('id', request.user_id);
    ...
    const { data: userProfile } = await db.from('profiles').select('referred_by').eq('id', request.user_id).single();
    if (userProfile?.referred_by) {
        // نتحقق إذا كان هذا أول شحن للمستخدم
        const { count } = await db.from('payment_requests').select('*', { count: 'exact', head: true }).eq('user_id', request.user_id).eq('status', 'approved');

        if (count === 1) { // هذه هي العملية الأولى الناجحة
            const { data: referrer } = await db.from('profiles').select('credits').eq('id', userProfile.referred_by).single();
            if (referrer) {
                await db.from('profiles').update({ credits: referrer.credits + 100 }).eq('id', userProfile.referred_by);
...
export const createVideoJob = async (userId: string, jobData: any) => {
    const { data, error } = await db.from('video_jobs').insert([{ ...jobData, user_id: userId }]).select().single();
    if (error) throw error;
    return data;
};
...
export const getUserVideoJobs = async (userId: string) => {
    const { data } = await db.from('video_jobs').select('*').eq('user_id', userId).order('created_at', { ascending: false });
    return data || [];
};
