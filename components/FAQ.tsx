        ...
        <div className="w-full max-w-5xl mx-auto py-10 animate-in fade-in duration-500 text-right">
            ...
            <div className="flex justify-center mb-10">
                <div className="bg-white/5 p-1.5 rounded-2xl border border-white/10 flex gap-2">
                    <button onClick={() => setActiveTab('faq')} className={`px-8 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'faq' ? 'bg-[var(--color-accent)] text-white shadow-lg' : 'text-white/40 hover:text-white'}`}>الأسئلة الشائعة</button>
                    <button onClick={() => setActiveTab('credits')} className={`px-8 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'credits' ? 'bg-[var(--color-accent)] text-white shadow-lg' : 'text-white/40 hover:text-white'}`}>تكاليف الخدمات</button>
                    <button onClick={() => setActiveTab('specs')} className={`px-8 py-2.5 rounded-xl text-xs font-black transition-all ${activeTab === 'specs' ? 'bg-[var(--color-accent)] text-white shadow-lg' : 'text-white/40 hover:text-white'}`}>المواصفات الفنية</button>
                </div>
            </div>
            ...
            {activeTab === 'faq' && (
                <div className="space-y-4 max-w-3xl mx-auto">
                    {FAQ_DATA.map((item, i) => (
                        <div key={i} className={`glass-card rounded-[2rem] overflow-hidden border transition-all ${activeIndex === i ? 'border-[var(--color-accent)]/30 bg-indigo-500/5' : 'border-white/5'}`}>
                            <button onClick={() => setActiveIndex(activeIndex === i ? null : i)} className="w-full p-8 text-right flex items-center justify-between flex-row-reverse">
                                <span className={`text-lg font-black ${activeIndex === i ? 'text-[var(--color-accent)]' : 'text-white/80'}`}>{item.q}</span>
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center border transition-all ${activeIndex === i ? 'rotate-180 bg-[var(--color-accent)] border-none shadow-lg' : 'border-white/10'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4">
                    {CREDIT_STRUCTURE.map(item => (
                        <div key={item.key} className="glass-card p-6 rounded-[2.5rem] border border-white/5 flex flex-col items-center gap-4 text-center">
                            ...
                            <div className="px-4 py-1.5 bg-indigo-500/10 text-indigo-400 rounded-full text-xs font-black">
            ...
            {activeTab === 'specs' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-bottom-4">
                    <div className="glass-card p-8 rounded-[3rem] border border-white/5 space-y-8">
                        <section>
                            <h3 className="text-[var(--color-accent)] font-black text-xs uppercase tracking-widest mb-4">أنماط الإضاءة المدعومة</h3>
                        ...
                        <section>
                            <h3 className="text-[var(--color-accent)] font-black text-xs uppercase tracking-widest mb-4">زوايا الكاميرا المتاحة</h3>
                            <div className="flex flex-wrap gap-2 justify-end">
